...
from .dedupe import _content_excerpt
...
이 후보들은 이미 1차(제목 유사도)·2차(Gemini 본문 분석) 중복·과거 기사 필터를
거쳤지만, 두 필터 모두 완벽하지 않으므로 아래 규칙에서 같은 판단을 마지막
안전망으로 다시 한번 적용하라.
...
2. 같은 사건·발표를 다룬 기사는 제목이 서로 크게 다르더라도(예: 같은 사건의
   ...
   병합한다. content(본문 발췌)까지 반드시 참고해 실제로 같은 사건인지
   판단하라 — 제목만으로 판단이 애매하면 본문의 인물명·사건명·수치가 겹치는지
   확인한다. 남길 기사는 본문이 더 상세하거나 주요 언론사인 쪽을 택한다.
3. 독자와 무관한 기사(연예, 스포츠, 단순 주가·시황, 광고성, 지역 단신 등)는 탈락시킨다.
4. 과거 기사 탈락: content(본문 발췌)에 오늘이 아닌 명확한 과거 날짜가 사건
   발생 시점으로 명시되어 있고, 그 사건에 대한 새로운 진전·반응·후속 내용
   없이 단순 재정리·재보도라면 탈락시킨다. 단순 배경 설명으로 과거를 언급하는
   기사, 오늘 나온 후속 진전·반응·분석 기사는 탈락 대상이 아니다. 애매하면
   탈락시키지 말고 남긴다(과탈락 방지).
5. 섹션 내 순서는 중요도(정책적 파급력, 독자 관련성) 순으로 정렬한다.
6. 섹션별 최대 기사 수: {limits}
7. 판결 기사(rulings)는 구체적 판결·결정 내용이 있는 기사만. 기관 동향은 law_judiciary_agencies로.
8. 결과는 섹션 키별 기사 id 배열(JSON)과 daily_summary 필드로 출력한다. 존재하지 않는 id를 만들지 마라.
9. daily_summary: 오늘 전체 기사 중 가장 중요한 흐름을 하나의 자연스러운 한국어 문장으로
