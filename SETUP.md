...
| 시각 (KST) | 동작 |
|---|---|
| 05:27 / 05:35 / 05:45 / 05:55 | 본 실행 시도 4회 분산 — GitHub Actions `schedule` 이벤트 자체의 지연(수십~90분+)을 흡수하기 위함. 가장 먼저 지연 없이 걸린 1회만 실제로 생성·발송하고, 나머지는 오늘자 아카이브가 이미 있음을 확인하고 1초 내 스킵(중복 발송 없음) |
| (실제 시작 시각 기준) | 수집(24시간 이내 기사만)·중복제거·분류·조판, 아카이브 커밋 |
| 06:28 | 발송 목표 시각(카카오 요약+링크와 Notion 동기화 동시 실행, 텔레그램 전문). 이 시각을 넘겨 시작된 경우 생성 완료 즉시 발송 |
| 06:45 | 백업 실행 — 4회 시도가 모두 실패/미발화한 경우의 마지막 안전망. 이미 발송됐으면 즉시 종료, 아니면 생성 후 즉시 발송 |
