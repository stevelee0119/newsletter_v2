...
- **06:30 수신 보장 설계**: GitHub Actions `schedule` 이벤트 자체의 지연(수십~90분+, `workflow_dispatch`보다 훨씬 느린 별도 큐)을 흡수하기 위해 05:27~05:55 사이 4회 분산 시도(가장 먼저 지연 없이 걸린 1회만 실행, 나머지는 아카이브 존재 확인 후 스킵) 후 06:28에 발송, 06:45 백업 실행으로 이중화
- **무인 운영**: 카카오 액세스 토큰은 매 실행 자동 갱신, 리프레시 토큰 회전 시 GitHub Secret도 자동 재저장

## 동작 방식

```
05:27/05:35/05:45/05:55 KST  GitHub Actions 실행 시도 (4회 분산 — 가장 먼저 지연 없이 걸린 1회만 아래 진행, 나머지는 스킵)
...
06:45 KST  백업 실행 (4회 시도가 모두 실패/미발화한 경우에만 생성 후 즉시 발송)
```
